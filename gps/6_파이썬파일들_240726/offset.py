...
# CSV 파일 읽기
real_data = pd.read_csv('ANUGROUND_REAL.csv')
gps_data = pd.read_csv('gps_data_0724_ANUslow1.csv')
...
# 오프셋 계산
offset_x = gps_data['x'].mean() - real_data['x'].mean()
offset_y = gps_data['y'].mean() - real_data['y'].mean()

# GPS 데이터 보정
gps_data['x'] = gps_data['x'] - offset_x
gps_data['y'] = gps_data['y'] - offset_y
...
plt.xlabel('X (meters)')
plt.ylabel('Y (meters)')
plt.title('GPS Data vs Real Data (After Offset Correction)')
